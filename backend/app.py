...
def api_create_patient():
    ...
    cur.execute("INSERT INTO patients (physio_id, name, dob, created_at) VALUES (?, ?, ?, ?)", (physio_id, name, dob, now))
    pid = cur.lastrowid
    conn.commit()
    conn.close()
...
def upload_file():
    ...
    conn = get_db_conn()
    cur = conn.cursor()
    cur.execute("SELECT id FROM patients WHERE id = ? AND physio_id = ?", (patient_id, session["physio_id"]))
    if cur.fetchone() is None:
        conn.close()
    ...
    if file and allowed_file(file.filename):
        ...
        file.save(save_path)
        try:
            out_image_path, stats = analyze_csv(save_path, app.config["OUTPUT_FOLDER"])
            now = datetime.utcnow().isoformat()
